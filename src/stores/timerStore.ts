  ...
  persist(
    (set, get) => ({
      ...
      resumeTimer: () => {
        ...
        // ✅ Recalculate startTime to maintain continuity
        // New startTime = now - secondsElapsed
        const now = new Date();
        const resumedStartTime = new Date(now.getTime() - state.secondsElapsed * 1000).toISOString();
